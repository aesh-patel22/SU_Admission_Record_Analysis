...
def get_verified_count(df: pd.DataFrame) -> int:
    """Count genuinely verified submitted applications from the source data.
    A record is verified only when Status is Submitted/Submit (excluding Not Submitted)
    AND the source 'Verified' column is Yes/True/1.
    """
    ...
    if 'Status' in df.columns:
        ...
        is_submitted = pd.Series(True, index=df.index)
...
if page == "🏠 Home Overview":
    ...
    if not df.empty:
        ...
        st.caption(
            ...
            f"Verified = Status is Submitted/Submit (excluding Not Submitted) AND Verified = Yes/True/1. "
            f"Across all 4 academic years (2023‑24 to 2026‑27): "
            f"{verified:,} ÷ {submitted:,} × 100 = {rate_ver}%."
        )
